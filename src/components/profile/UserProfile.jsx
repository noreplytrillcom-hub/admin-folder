...
export default function UserProfile({ initialUser, onSave }) {
  // Navigation Tabs state: "general" | "security" | "preferences"
  ...
  // Pristine Saved State for unsaved changes detection
  ...
  // Toast Notification State (styled with #EAE8FA bg & #7952F5 text)
  ...
  const handlePhotoUpload = (e) => {
    ...
    reader.onloadend = () => {
      ...
      setToast({
        type: "success",
        text: "Profile picture preview updated! Click 'Save Changes' to finalize."
      });
    };
  ...
  const handleRemovePhoto = () => {
    ...
    setToast({
      type: "info",
      text: "Photo removed. Default initials fallback displayed."
    });
  };
  ...
  const handleSaveGeneralInfo = async (e) => {
    ...
    try {
      ...
      setToast({
        type: "success",
        text: "User profile updated successfully! All changes persisted."
      });
    } catch (err) {
      setToast({
        type: "error",
        text: "Failed to connect to API server. Please try again."
      });
    } finally {
  ...
  const handleDiscardGeneralChanges = () => {
    ...
    setToast({
      type: "info",
      text: "Form reset to pristine saved state."
    });
  };
  ...
  const handleUpdatePassword = async (e) => {
    ...
    if (securityData.newPassword !== securityData.confirmPassword) {
      secErrors.confirmPassword = "New password and confirmation password do not match.";
    }
    ...
    if (Object.keys(secErrors).length > 0) {
      ...
      setToast({
        type: "error",
        text: "Password security requirements are not met."
      });
      return;
    }
  ...
  return (
    <div
      className="w-full min-h-screen bg-[#F8F8FC]"
      style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}
    >
      {/* Outer Content Frame */}
      <div className="w-full max-w-7xl mx-auto px-8 py-6 space-y-6">

        {/* TOAST NOTIFICATION BANNER */}
        {toast && (
          <div
            className="fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl border border-[#D1B9FE] bg-[#EAE8FA] text-[#7952F5] transition-all duration-300 transform translate-y-0 max-w-md"
          >
        ...
        {/* 2. NAVIGATION TAB BAR: mt-6 mb-6 flex border-b border-gray-200 pb-3 */}
          ...
          <div className="bg-[#FFFFFF] border border-[#EAE8FA] rounded-[16px] p-6 sm:p-8 shadow-sm space-y-6">
            ...
            <form onSubmit={handleSaveGeneralInfo} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {/* First Name - pl-10 mandatory left padding to prevent icon collision */}
                ...
                {/* Last Name - pl-10 mandatory left padding */}
                <div className="space-y-1">
                  <label className="block mb-1 text-xs font-bold uppercase tracking-wider text-[#18112B]/80">
                ...
                {/* Email Address - pl-10 mandatory left padding */}
                <div className="space-y-1">
                  <label className="block mb-1 text-xs font-bold uppercase tracking-wider text-[#18112B]/80">
                ...
                {/* Phone Number - pl-10 mandatory left padding */}
                <div className="space-y-1">
                  <label className="block mb-1 text-xs font-bold uppercase tracking-wider text-[#18112B]/80">
                    Phone Number
                  </label>
        ...
        {/* ------------------------------------------------------------- */}
        {/* TAB 2: SECURITY & PASSWORD SECTION */}
        {/* ------------------------------------------------------------- */}
        {activeTab === "security" && (
        ...
        {/* ------------------------------------------------------------- */}
        {/* TAB 3: NOTIFICATION PREFERENCES SECTION */}
        {/* ------------------------------------------------------------- */}
        {activeTab === "preferences" && (
